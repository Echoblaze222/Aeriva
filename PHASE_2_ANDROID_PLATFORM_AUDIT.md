...
measurement engine in the repository yet.
...
| Cellular signal strength | `TelephonyManager.PhoneStateListener` / `TelephonyCallback` (`SignalStrength`) | 26+ (`TelephonyCallback` is 31+) | `READ_PHONE_STATE` for some signal-strength paths depending on API level (not independently re-verified in this pass; flagged as a known unknown, Section 13) | Yes, callback-driven | **Yes** -- emulators report synthetic/absent cellular signal | Low | None | Low-medium -- OEM/radio dependent | Needs its own dedicated, narrower research pass before implementation; not fully verified in this audit |
...
- **API-level-gated capabilities relevant to this audit specifically:**
  ...
  (34+). Each of these needs either a minSdk-26-compatible fallback or
  an explicit "not supported below API X" decision -- this audit does
  not make that decision, it surfaces that the decision exists.
...
- Cellular `TelephonyManager`/`TelephonyCallback` signal-strength
  permission requirements were not independently re-verified this
  session to the same depth as the Wi-Fi/location findings -- flagged
  explicitly in Section 3's own row rather than stated with unearned
  confidence, and listed again in Section 13.
...
- Real cellular signal-strength reporting and its exact permission
  requirements across API 26 through 36 (Section 12).
...
1. `WorkManager` is added as a dependency and at least one real,
...
2. A written, specific justification exists for any location permission
   ...
   does this serve."
3. The OBSERVATION/MEASUREMENT/ESTIMATION/PREDICTION/RECOMMENDATION
   ...
   this document.
4. A decision is made and recorded on whether `PACKAGE_USAGE_STATS`
   (device-wide usage) is in scope at all, separate from and later than
   own-app usage (which needs no such decision to proceed).
5. At least the highest-risk findings in Section 13 (real Wi-Fi
   behavior, real Doze behavior, real OEM battery-optimization behavior
   on one concretely-named device/ROM) have been tested on physical
   hardware, with results recorded, before any measurement-engine or
   background-monitoring feature building on them is considered
   validated.
6. This document's own Section 1 finding -- that the existing codebase
