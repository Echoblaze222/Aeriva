...
| **Wi-Fi characteristics** (SSID/BSSID/RSSI) | `WifiManager.getConnectionInfo()`/`WifiInfo` | `ACCESS_FINE_LOCATION` at every currently-supported API level (current official Wi-Fi-permissions docs), plus device location services enabled | Foreground-only without additionally requesting background location (not planned) | Available since API 1, restricted progressively since 26/29 | SSID/BSSID/RSSI for the currently-connected network, when granted | Scan results for networks not currently connected (needs the separate, more-throttled `startScan()` path, not designed here) | Real SSID/RSSI behavior is real-radio-dependent; user can deny permission or disable location entirely | **Yes, specifically** -- emulator Wi-Fi is virtual | No, not meaningfully |
...
| `WifiManager` reads (if capability 7 is ever implemented) | No persistent registration for `getConnectionInfo()` itself (a single synchronous call) -- only `startScan()`/scan-results broadcast (not designed here) would need registration | N/A for the single-call form | N/A for the single-call form | Each call should be made freshly when needed, not cached long-term (Wi-Fi state changes) | Low, for the single-call form designed here | A rapid Wi-Fi transition (e.g. AP roam) between the connectivity callback firing and a `getConnectionInfo()` call could read stale info -- a future provider must read fresh `WifiInfo` at the moment of use, not cache it across the transition |
